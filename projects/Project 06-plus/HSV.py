...
:function: Color Space-based Face Detection (HSV Space)
:author: Fu Tszkok
:date: 2025-02-06
...
# Convert the image from BGR to HSV. This separates color information (Hue and
# Saturation) from brightness (Value), making color-based segmentation more
# reliable under varying lighting conditions.
hsv_image = cv.cvtColor(image, cv.COLOR_BGR2HSV)

# Define the lower and upper bounds of the target color in HSV space.
# These values correspond to a specific color range to be detected.
lower_hsv = (0, 20, 60)
upper_hsv = (50, 150, 255)

# Apply a Gaussian blur to the HSV image to reduce noise before thresholding.
...
# HSV range to white (255) and all other pixels to black (0).
...
mask_cleaned = cv.morphologyEx(mask_hsv, cv.MORPH_CLOSE, kernel)
...
for contour in tqdm(contours, desc="Processing contours"):
    ...
    if cv.contourArea(contour) > 1000:
        ...
        aspect_ratio = w / h
        # Filter for objects that are approximately square or circular.
